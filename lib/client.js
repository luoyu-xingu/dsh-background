window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 运行时调整蒙层强度只改这个变量,不重写样式表、不换图片 URL——
		// no-store 图片若被规则重应用会被重新拉取,表现为滑块每次滑动闪一下。
		// 另修复输入框停靠区遮罩(composerSeat):其原渐变色消费 color-mix(bg-base),
		// 变量带 url 时失效变透明导致滚动文字透到输入框下方——在座位 ::before 上
		// 铺与页面一致的背景(图+蒙层,fixed 视口对齐),顶部 36px 用 mask 渐隐;
		// 不用纯色渐隐,否则输入框下方会被盖成白块,破坏背景连续性。
		...
		class BackgroundClient {
			...
			adopt(section) {
				...
				if (path !== this.currentPath) {
					...
					this.imageRev += 1;
					this.ensureStyleEl();
					// 样式表只在路径变化(图片 URL 变化)时重写;蒙层强度单独走
					// CSS 变量,重写样式表会触发 no-store 图片重新拉取(闪动)。
			...
			/** 把蒙层强度写到 CSS 变量(不重写样式表、不换图片 URL)。 */
			applyVeil(veil) {
				document.documentElement.style.setProperty("--dshbg-veil-alpha", veilAlpha(veil));
			}
		...
		function BackgroundRow({ t, useStore, setPath, setVeil, clear }) {
			...
			// 草稿只在持久化状态真正变化时回填;交互标志(focused / veilActive)
			// 不能进依赖数组——松手/失焦瞬间标志翻转而写入尚在途,会把草稿
			// 先回填成旧值(滑块跳回原处),写入落盘后又跳回新值。
			react.useEffect(() => {
		...
		function apply(ctx) {
			...
			let bound;
			// 设置行组件通常在配置加载完成后才挂载(刷新后页面先停在会话页,
			// 设置页按需打开):此时 bound 尚未注入,推送会被静默丢弃,导致打开
			// 设置页看到空路径、空预览——而背景本身由宿主端启动注入,不受影响。
			// 因此缓存最近一次加载结果,行组件挂载时补投进 store。
			let latest = null;
			let lastError = null;
			...
			ctx.effect(() => {
				client.load().then(
					...
					(error) => fail("dsh-background: config load failed", error)
				);
			}, "dsh-background: initial config load");

			const injected = (actions) => {
				if (actions) bound = actions;
				// 行组件可能晚于初始加载才挂载:补投最近一次加载结果(或错误),
				// 否则刷新后打开设置页时路径输入框与预览是空的。
				if (latest !== null && actions) actions.sync(latest);
				else if (lastError !== null && actions) actions.fail(lastError);
				return {
					setPath: (value) => {
