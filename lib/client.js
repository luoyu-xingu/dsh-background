window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 图片之上按主题叠加半透明蒙层(浅色白蒙层 / 深色深蒙层),保证任意亮度
		// 图片上应用文字可读;作用域限定 html[data-dsh-background],不影响默认外观。
		function buildBackgroundCss(imageUrl) {
			const image = `url(${JSON.stringify(imageUrl)})`;
			const veilLight = "linear-gradient(rgba(255, 255, 255, 0.75), rgba(255, 255, 255, 0.75))";
			const veilDark = "linear-gradient(rgba(9, 11, 15, 0.7), rgba(9, 11, 15, 0.7))";
