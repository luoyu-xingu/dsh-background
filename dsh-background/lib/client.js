window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *    清除按钮,以及 180×88 的实时预览(尺寸参照默认「外观」主题立方体);
		...
		function buildBackgroundCss(imageUrl) {
			const image = `url(${JSON.stringify(imageUrl)})`;
			const stack = `${image} center / cover no-repeat fixed`;
			return [
				`html[data-dsh-background] body{--dsw-alias-bg-base:${stack}, var(--dsw-static-neutral-bluish-00);}`,
				`html[data-dsh-background] body[data-ds-dark-theme]{--dsw-alias-bg-base:${stack}, var(--dsw-static-neutral-bluish-950)}`
			].join("\n");
		}
		...
		class BackgroundClient {
			...
			/** 写路径配置(宿主进程内持久化到 settings.yaml)。 */
			async writePath(path) {
				const result = await this.ctx.connection.rpc.call("/dsh-background", "config", { op: "set", path });
			...
			adopt(section) {
				...
				if (path === "") {
					...
					this.ensureStyleEl();
					this.styleEl.textContent = buildBackgroundCss(`/dsh-background/image?v=${this.rev}`);
					document.documentElement.setAttribute("data-dsh-background", "");
				}
				return { path, rev: this.rev };
			}
		...
		function createBackgroundRowStore() {
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					sync: (d, next) => {
						if (d.path === next.path && d.rev === next.rev && d.status === next.status) return;
						d.path = next.path;
						d.rev = next.rev;
		...
		function BackgroundRow({ t, useStore, setPath, clear }) {
